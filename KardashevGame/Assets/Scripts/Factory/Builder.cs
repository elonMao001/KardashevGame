...
public class Builder : MonoBehaviour
{
    ...
    //�berpr�ft, was f�r einen Collider ein RayCast zur Maus ber�hrt, und startet dann je nach selected den Bau eines bestimmten Geb�udes
    void CheckRay()
    {
        RaycastHit hit;
    ...
    //Erstellt ein Fabrik-GameObject in der richtigen Position und Gr��e, sowie mit den richtigen Komponenten
    ...
    //Erstellt ein Conveyor-GameObject in der richtigen Position und Gr��e und h�ngt ein ConveyorCheater-Skript dran
    ...
    //Coroutine, welche �berpr�ft, welcher Conveyor-Fall vorliegt und DrawConveyor() mit dem richtigen Conveyor-Skript startet
    private IEnumerator ConstructConveyor(RaycastHit hit1)
    {
        while (true)
        {
            ...
            if (hit1.collider.gameObject.CompareTag("Factory"))
            {
                ...
                if (isOut)
                {
                    c = new Conveyor(/*speed*/ 10, v1, hit2.point + new Vector3(0, 1, 0), f1, false); //Achtung: M�glicherweise wurden hier input und output vertauscht
                }
                else
                {
    ...
    //Umst�ndlicher "tempor�rer" Code, um aus einem Tastatur-Event den char f�r selected zu w�hlen
